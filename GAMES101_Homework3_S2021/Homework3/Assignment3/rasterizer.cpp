...
Eigen::Vector3f getNormalBy3Points(
...
{
    auto box = getboxaabb(t.toVector4());
    auto start = box[0];
    auto end = box[1];
    ...
    for (int x = start.x(); x < end.x() + 1; x++)
    {
        for (int y = start.y(); y < end.y() + 1; y++)
        {
            ...
            if (depth_buf[ind] >= zp) //深度结果是负数 初始化是无穷大 那值越小权重越大
            {
                // float r = alpha * t.color[0].x() + beta * t.color[1].x() + gamma * t.color[2].x();
                // float g = alpha * t.color[0].y() + beta * t.color[1].y() + gamma * t.color[2].y();
                // float b = alpha * t.color[0].z() + beta * t.color[1].z() + gamma * t.color[2].z();

                // float u = alpha * t.tex_coords[0].x() + beta * t.tex_coords[1].x() + gamma * t.tex_coords[2].x();
                // float v = alpha * t.tex_coords[0].y() + beta * t.tex_coords[1].y() + gamma * t.tex_coords[2].y();

                // float normal_x = alpha * t.normal[0].x() + beta * t.normal[1].x() + gamma * t.normal[2].x();
                // float normal_y = alpha * t.normal[0].y() + beta * t.normal[1].y() + gamma * t.normal[2].y();
                // float normal_z = alpha * t.normal[0].z() + beta * t.normal[1].z() + gamma * t.normal[2].z();
                // Eigen::Vector3f interpolated_color={r,g,b};
                // Eigen::Vector2f interpolated_texcoords={u,v};
                // Eigen::Vector3f interpolated_normal={normal_x,normal_y,normal_z};
                ...
                depth_buf[ind] = zp;
                Eigen::Vector2i drawpoint(x, y);
                set_pixel(drawpoint, pixel_color);
            }
        }
    }
